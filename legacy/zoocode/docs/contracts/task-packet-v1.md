...
Every later packet must refer to `RAW_INPUT_REF_V1` artifacts by path and preserve `routing_control.completion_unwind` when present. Intermediate handoffs that require another mode must include `handoff_status: requires_parent_dispatch`, `workflow_complete: false`, `recommended_next_mode: <next-mode>`, and `next_action: {type: new_task, tool: new_task, mode: <next-mode>}` so Roo/Zoo runtimes can dispatch the required Boomerang `new_task` with its `mode` parameter and do not treat the handoff as final workflow completion. Every nonterminal intake-chain handoff, not only raw materialization, must include this pair. `recommended_next_mode` and `next_action.mode` must name the same immediate target; `next_action` or prose alone is insufficient for nonterminal Boomerang continuation.
