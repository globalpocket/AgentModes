...
- When writing visible todos in Zoo/Roo, send a multi-line TODO body with actual newline characters between the heading and each checklist item; never serialize the TODO as a single escaped string and never emit literal `\n` escape sequences inside the todo text.
- Use `VISIBLE_TODO_V1` (`docs/contracts/visible-todo-v1.md`) for structured TODO handoffs whenever a controller or workflow hands off TODO state: keep `title` and `items` separate until the final Zoo/Roo rendering boundary.
