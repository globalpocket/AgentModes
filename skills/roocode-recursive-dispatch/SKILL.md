...
- 実装、テスト、Coverage 85%以上、security-auditor、reviewer、必要な Pull Request / 診断登録が完了している。
...
if (fs.existsSync(historyFile) && fs.readFileSync(historyFile, "utf8").includes(fingerprint)) process.exit(0)
...
1. .roo/recursive-dispatch/dispatch.log に IPC Ack timeout または IPC socket is ambiguous or missing がないことを確認する。
2. .roo/recursive-dispatch/history.jsonl に今回 fingerprint が追記されていることを確認する。
3. 失敗時も現在のメインタスク完了結果を取り消さず、Recursive Dispatch failed として完了報告に残す。
