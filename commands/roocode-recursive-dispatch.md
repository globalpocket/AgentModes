...
- 実装、テスト、Coverage 85%以上、security-auditor、reviewer、release-manager、diagnostic-reporter、必要な issue-tracker 完了コメントが完了済みである。
...
- Parent Task Summary: 最終統合結果、Pull Request URL、Diagnostic Issue URL、テスト・Coverage・監査結果を最大12行へ圧縮したもの。

## 実行手順

1. 最初に skill ツールで `roocode-recursive-dispatch` をロードして実行する。
2. Skill実行環境が使えない場合だけ、`skills/roocode-recursive-dispatch/SKILL.md` の手順をSkill本体として扱い、`.roo/recursive-dispatch/next-prompt.md` と `.roo/recursive-dispatch/parent-summary.md` を作成して Async Dispatch Command を実行する。
3. VSCode外CLI agent、`roo --print`、`roo --stdin-prompt-stream`、TUI起動は使わない。IPC `StartNewTask` のみを使う。
4. socket 不明、socket 曖昧、Ack timeout、duplicate fingerprint、深度上限到達時は skipped / failed として記録し、現在のメインタスク完了を取り消さない。
5. 実行後は `.roo/recursive-dispatch/dispatch.log` と `.roo/recursive-dispatch/history.jsonl` の軽量確認だけを行い、次タスク完了を待たない。
