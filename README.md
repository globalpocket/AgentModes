...
- `release-manager` は、GitHub由来リポジトリでのみメインタスク開始時の `release` ブランチ準備と、テスト、Coverage 85%以上、security-auditor、reviewer の品質ゲート通過後のGitHub MCP公開手順だけを担当し、Issue Contextがある場合はトピックブランチとPull Requestを元Issueへ紐づける。GitHub Issueの自動クローズは、closing keywordを含むPRが既定ブランチへmergeされた場合だけ発火し、PRをcloseしただけ、または既定ブランチではない `release` 宛てPRだけでは発火しないため、Auto-Close EligibilityをPR本文・元Issueコメント・完了報告へ明記する
