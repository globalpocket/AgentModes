...
Sliding-window operation must keep raw large input in artifacts rather than conversation history. The raw body may appear only in the one `RAW_INPUT_PAYLOAD` handoff to `raw-input-materializer`; once materialized, downstream modes use `RAW_INPUT_REF_V1`, `raw_request_path`, `USER_NEEDS_V1`, `SESSION_START_V1`, `SESSION_CURSOR_V1`, and `RUN_STATE_V1` paths as the source of truth.
