...
The `raw-input-materializer` mode is a fallback for large input that still fits in the active request; it is not the root defense against provider-context overflow.
